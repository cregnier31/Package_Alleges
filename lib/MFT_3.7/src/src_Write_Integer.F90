...
! -- SUBROUTINE MIOL_writef_field_I4_scalar_NC (cd_filename,cd_varname, id_varvalue,ida_specialvalue)
! -- SUBROUTINE MIOL_writeu_field_I4_scalar_NC (id_file_id,cd_varname,id_varvalue,ida_specialvalue)
 !******************************************************************************
       ...
       SUBROUTINE MIOL_writef_field_I4_4D_NC (cd_filename, &
         ...
         cl_fonction="MIOL_writef_field_I4_4D_NC"
               ...
               DO il_jk=1, il_nbfiledim
                  ...
                  IF (TRIM(cla_dimname(il_jj)) .EQ. TRIM(cla_filedimname(il_jk))) THEN
                     ...
                     IF (ila_filedimlen(il_jk).NE.SIZE(ida_varvalue, dim=il_ji)) THEN
                        WRITE(0,*) ' MIOL_write_field_I4_4D_NC : dimensions error. '
                        CALL flush(0)
                        STOP
                     ENDIF
  ...
  !******************************************************************************
       ...
       SUBROUTINE MIOL_writef_field_I4_3D_NC (cd_filename, &
         ...
         cl_fonction="MIOL_writef_field_I4_3D_NC"
            ...
            DO il_jj=1, il_kindofdim
               ...
               DO il_jk=1, il_nbfiledim
                  ...
                  IF (TRIM(cla_dimname(il_jj)) .EQ. TRIM(cla_filedimname(il_jk))) THEN
                     ...
                     IF (ila_filedimlen(il_jk).NE.SIZE(ida_varvalue, dim=il_ji)) THEN
                        WRITE(0,*) ' MIOL_write_field_I4_3D_NC : dimensions error. '
                        CALL flush(0)
                        STOP
                     ENDIF
  ...
  !******************************************************************************
       ...
       SUBROUTINE MIOL_writef_field_I4_2D_NC (cd_filename, &
         ...
         cl_fonction="MIOL_writef_field_I4_2D_NC"
            ...
            DO il_jj=1, il_kindofdim
               ...
               DO il_jk=1, il_nbfiledim
                  ...
                  IF (TRIM(cla_dimname(il_jj)) .EQ. TRIM(cla_filedimname(il_jk))) THEN
                     ...
                     IF (ila_filedimlen(il_jk).NE.SIZE(ida_varvalue, dim=il_ji)) THEN
                        WRITE(0,*) ' MIOL_write_field_I4_2D_NC : dimensions error. '
                        CALL flush(0)
                        STOP
                     ENDIF
 ...
 !******************************************************************************
       ...
       SUBROUTINE MIOL_writef_field_I4_1D_NC (cd_filename, &
         ...
         cl_fonction="MIOL_writef_field_I4_1D_NC"
            ...
            DO il_jj=1, il_kindofdim
               ...
               DO il_jk=1, il_nbfiledim
                  ...
                  IF (TRIM(cla_dimname(il_jj)) .EQ. TRIM(cla_filedimname(il_jk))) THEN
                     ...
                     IF (ila_filedimlen(il_jk).NE.SIZE(ida_varvalue, dim=il_ji)) THEN
                        WRITE(0,*) ' MIOL_write_field_I4_1D_NC : dimensions error. '
                        CALL flush(0)
                        STOP
                     ENDIF
 ...
 !******************************************************************************
       ...
       SUBROUTINE MIOL_writef_field_I4_scalar_NC (cd_filename, &
         ...
         INTEGER :: il_ji,il_status, il_kindofdim,il_lon,il_lat,il_false_nth, &
                    il_standard,il_false_sth,il_semi_major_axis,il_semi_minor_axis
         LOGICAL :: ll_tri
         ...
         cl_fonction='MIOL_writef_field_I4_scalar_NC'
         IF (cl_miolUserParameterFile /= '') THEN 
            OPEN(20, file=cl_miolUserParameterFile, &
                 status='old', &
                 form='formatted')
         ...
         cl_varfile = TRIM(cl_variableAttributesPath)//TRIM(cd_varname)//'.in'
         print *,"Read path :",cl_varfile 
         il_lon=99999
         il_lat=99999
         il_false_nth=99999
         il_false_sth=99999
         il_standard=99999
         il_semi_major_axis=99999
         il_semi_minor_axis=99999
         ...
         IF ( (PRESENT(ida_specialvalue))) THEN
            il_lon = ida_specialvalue(1)
            il_lat = ida_specialvalue(2)
            il_standard = ida_specialvalue(3) 
            il_false_sth = ida_specialvalue(4)
            il_false_nth = ida_specialvalue(5)
            il_semi_major_axis = ida_specialvalue(6)
            il_semi_minor_axis = ida_specialvalue(7)
         ENDIF
         ...
         DO il_ji = 1, il_nbatt
            ...
            SELECTCASE (cl_attvalue)
               ...
               CASE ('straight_vertical_longitude_from_pole')
                  il_status = fi_ncError(NF90_PUT_ATT(il_file_id, &
                                                      il_varin_id, &
                                                      cl_attname, &
                                                      il_lon),cl_fonction)
               ...
               CASE ('semi_major_axis') 
                  il_status = fi_ncError(NF90_PUT_ATT(il_file_id, & 
                                                      il_varin_id, & 
                                                      cl_attname, & 
                                                      il_semi_major_axis),cl_fonction) 
               CASE ('semi_minor_axis') 
                  il_status = fi_ncError(NF90_PUT_ATT(il_file_id, & 
                                                      il_varin_id, & 
                                                      cl_attname, & 
                                                      il_semi_minor_axis),cl_fonction) 
       ...
       SUBROUTINE MIOL_writeu_field_I4_scalar_NC (id_file_id, &
         ...
         CHARACTER(LEN=*),            INTENT(IN) :: cd_varname
         INTEGER(KIND=4), INTENT(IN) :: id_varvalue
         INTEGER, DIMENSION(7),       OPTIONAL, INTENT(IN) :: ida_specialvalue
         CHARACTER(LEN=255) :: cl_equivalencesPath, cl_globalAttributesPath, &
                               cl_variableAttributesPath, cl_dimensionsPath, &
                               cl_varfile,cl_varname
         ...
         cl_fonction="MIOL_writeu_field_I4_scalar_NC"
         ...
         IF (ll_tri) THEN
            OPEN(20, file=cl_varfile, status='old', form='formatted')
            READ(20, nb_att)
            print *,'Varfile exist ',cl_varname
         !-----------------------------------------------------------------------
         ! Define the variable
  ...
  !******************************************************************************
       ...
       SUBROUTINE MIOL_writeu_field_I4_4D_NC (id_file_id, &
         ...
         cl_fonction="MIOL_writeu_field_I4_4D_NC"
  ...
  !******************************************************************************
       ...
       SUBROUTINE MIOL_writeu_field_I4_3D_NC (id_file_id, &
         ...
         cl_fonction="MIOL_writeu_field_I4_3D_NC"
         CALL flush(0)
  ...
  !******************************************************************************
       ...
       SUBROUTINE MIOL_writeu_field_I4_2D_NC (id_file_id, &
         ...
         cl_fonction="MIOL_writeu_field_I4_2D_NC"
  ...
  !******************************************************************************
       ...
       SUBROUTINE MIOL_writeu_field_I4_1D_NC (id_file_id, &
         ...
         cl_fonction="MIOL_writeu_field_I4_1D_NC"
